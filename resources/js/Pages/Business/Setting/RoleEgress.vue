  ...
  import { TextInput, SecondaryButton, Table, ConfirmationModal, PrimaryButton } from "@/Components";
  import { TrashIcon, PencilIcon } from "@heroicons/vue/24/solid";
  import { Errors, Filters, GeneralRequest, RoleEgress } from "@/types";

  const props = defineProps<{
    egresses: GeneralRequest<RoleEgress>;
    filters: Filters[];
  }>();
  ...
  // Reactives
  const egress = useForm<RoleEgress>({ ...initialEgress });
  ...
  const update = (egressEdit: RoleEgress) => {
    resetErrorForm();
  ...
  const removeegress = (egressId: number) => {
    toggle1();
...
<template>
  <BusinessSettingLayout title="Negocio ajustes de roles">
    ...
    <div class="w-full overflow-x-auto">
      ...
      <Table>
        ...
        <tbody>
          <tr v-for="(egress, i) in props.egresses.data" :key="egress.id" class="border-t [&>td]:py-2">
            <td>{{ i + 1 }}</td>
            <td>{{ egress.code }}</td>
            ...
            <td class="flex justify-end">
              <div class="relative inline-flex gap-1">
                <button v-if="egress.type === 'otro'" class="rounded px-1 py-1 bg-red-500 text-white"
                  @click=" ()=> egress.id && removeegress(egress.id)">
                  <TrashIcon class="size-6 text-white" />
                </button>
                <button v-if="egress.type === 'otro'" class="rounded px-2 py-1 bg-blue-500 text-white"
                  @click="update(egress)">
  ...
  <ModalEgress :show="modalegreses" :egress="egress" :error="errorForm" @close="toggle" @save="save" />
